...
export class AutorService {
  ...
  UploadFile(file) {
    ...
    return this.http.post(`${url}containers/Articles/upload?access_token=${this.token}`, data);
  }
  ...
  UploadArticle(file): Observable<ArticleModel> {
    return this.http.post<ArticleModel>(`${url}Articles?access_token=${this.token}`, file, {
      headers: new HttpHeaders({
        "content-type": "application/json"
      })
    });
